...
import { 
  ...
  unvoteForSubmission // <-- add this import
...
const ChallengesPage = () => {
  ...
  const handleUnvote = async (recipeId) => {
    try {
      await unvoteForSubmission(selectedChallenge.id, recipeId);
      showToast('Vote removed');
      fetchLeaderboard(selectedChallenge.id);
    } catch (error) {
      console.error('Error unvoting:', error);
      showToast('Error removing vote');
    }
  };
  ...
  return (
    <div>
      ...
      >
        {/* Add mt-24 wrapper to push content below the header */}
        <div className="mt-24">
                            ...
                            <div className="space-y-3">
                              {leaderboard
                                ...
                                .map((submission, index) => {
                                  ...
                                  return recipe ? (
                                    <div key={submission.recipeId} className="border rounded-lg p-3 flex flex-col md:flex-row md:justify-between md:items-center gap-2">
                                      <div>
                                      ...
                                      <div className="flex gap-2 mt-2 md:mt-0 items-center">
                                        <button
                                          ...
                                          className="p-1 rounded-full bg-gray-100 hover:bg-blue-100 transition text-lg"
                                          style={{ width: 28, height: 28, minWidth: 28, minHeight: 28, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                          title="View Recipe"
                                        >
                                          {/* 👁️‍🗨️ emoji instead of image */}
                                          <span style={{ fontSize: 16, lineHeight: 1 }}>👁️‍🗨️</span>
                                        </button>
                                        {new Date(selectedChallenge.endDate) > new Date() && (
                                          hasVoted ? (
                                            <button
                                              onClick={() => handleUnvote(submission.recipeId)}
                                              style={{
                                            ...
                                            <button
                                              ...
                                              className="px-3 py-2 rounded-lg text-sm hover:bg-gray-800 transition"
                                            >
                                              Vote
                                            </button>
                                          )
