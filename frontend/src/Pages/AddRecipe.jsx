...
const AddRecipe = () => {
  ...
  // Generate preview for image uploads
  useEffect(() => {
    if (file) {
  ...
  // Format ingredients with automatic bullets
  ...
  // Format instructions with automatic step numbering
  const formatInstructions = (text) => {
    ...
    return lines
      .map(line => {
        ...
        // Remove any existing step prefix and capitalize first letter
  ...
  useEffect(() => {
    const generateRecipeId = () => {
      return 'RCP-' + Math.random().toString(36).substr(2, 9).toUpperCase();
    };
    setRecipeData(prev => ({
      ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    const toastId = toast.loading('Submitting your recipe...');
    ...
    try {
      ...
      toast.update(toastId, {
        render: 'Failed to submit recipe. Please try again.',
        type: 'error',
        isLoading: false,
        autoClose: 3000
      });
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          <p className="mt-3 text-xl text-gray-500">
            Fill in the details below to add your recipe to our collection
          </p>
        </motion.div>
